...
class J_Controller
{
    public:
        ...
        // constructor 
        J_Controller(double kp=0, double kd=0,double ki=0)
        {
        ...
        // PID operator
        double compute_JposPID(double target, double current_ang, double dt, double cutoff)
        {   
            
            double t_const= 1/(2*pi*cutoff); // time constant
            err[0] = target-current_ang;
            P_term_ = Kp_*err[0];
            D_term_[0] = (Kd_*2*(err[0]-err[1])-(dt-2*t_const)*D_term_[1])/(2*t_const+dt);
            I_term_[0] = I_term_[1] + Ki_*dt/2*(err[0] + err[1]);
